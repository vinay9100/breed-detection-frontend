...
def login(user_login: schemas.UserLogin, db: Session = Depends(get_db)):
    # Try finding the user with cleaned email first
    clean_email = get_clean_email(user_login.email)
    db_user = db.query(models.User).filter(models.User.email == clean_email).first()

    # Fallback to raw email lookup if not found (in case farmer registered with raw email)
    if not db_user:
        db_user = db.query(models.User).filter(models.User.email == user_login.email.strip()).first()

    if not db_user:
        raise HTTPException(status_code=404, detail="Email not registered")

    # Role Validation with clear error messages
    is_bpa_attempt = user_login.email.upper().startswith("BPA-")
    
    if is_bpa_attempt and db_user.role != "bpa":
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, 
            detail="Account role mismatch: This is a Farmer account."
        )
    if not is_bpa_attempt and db_user.role != "farmer":
        # Exception: Allow BPA to login without prefix if they want to access general features?
        # For now, let's stick to the rule but give a better message
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, 
            detail="Account role mismatch: BPA Officers must use 'BPA-' prefix."
        )
    ...
    try:
        password_is_valid = security.verify_password(user_login.password, db_user.password_hash)
    except Exception:
        pass
        
    if not password_is_valid:
        # Fallback for plain-text storage or edge cases
        if db_user.password_hash == user_login.password:
    ...
    if needs_hash_upgrade:
        db_user.password_hash = security.get_password_hash(user_login.password)
        db.commit()
        db.refresh(db_user)
...
def get_analytics(time_filter: str = "Week", current_user: models.User = Depends(security.get_current_user), db: Session = Depends(get_db)):
    ...
    if current_user.role == "bpa":
        # For BPA Officers: Growth Chart shows official registrations ONLY (not scans)
        reg_query = db.query(
        ...
        bar_results = reg_query.group_by('date').order_by('date').all()
